    ...
    public function searchUser(Request $request)
    {
        ...
        try {
            ...
            if ($request->role == "2") {
                ...
                // $staffs = SmStaff::with('staff_user','roles')->where('role_id', $request->role)->get();dd($staffs);
               
                 $staffs= DB::table('sm_staffs')
                  
                 ->leftjoin('infix_roles','sm_staffs.role_id','infix_roles.id')
                 ->where('role_id', $request->role)->get();                return view('backEnd.systemSettings.login_access_control', compact('staffs', 'role', 'roles', 'classes'));
            }
            return view('backEnd.systemSettings.login_access_control', compact('roles', 'classes'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
