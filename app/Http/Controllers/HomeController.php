...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        try {
            ...
            elseif ($role_id == 10) {
                return redirect('schoolModule-dashboard');
            }
             elseif ($role_id == "") {
    ...
    public function schoolModule()
    {
        ...
        return view('backEnd.schoolmodal',compact('school_info'));
    }
    public function districtPermissionUpdate(Request $request)
    {
        $sm_staf = DB::table('sm_staffs')->select('district_idFk')->where(['role_id'=>Auth::user()->role_id,'user_id'=>Auth::user()->id])->first();

        $infix_permission_assigns =  DB::table('infix_permission_assigns')->where(['role_id'=>Auth::user()->role_id,'district_idFk'=>$sm_staf->district_idFk])->update(array('school_id' => $request->id));
        $sm_staf = DB::table('sm_staffs')->where(['role_id'=>Auth::user()->role_id,'user_id'=>Auth::user()->id])->update(array('school_id' => $request->id));
        $sm_staf = DB::table('users')->where(['role_id'=>Auth::user()->role_id,'id'=>Auth::user()->id])->update(array('school_id' => $request->id));
