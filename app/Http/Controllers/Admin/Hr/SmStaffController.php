...
class SmStaffController extends Controller
{
    ...
    public function staffList(Request $request)
    {
        ...
        try {
            ...
            $all_staffs = $all_staffs->with('departments', 'designations', 'roles')->where('school_id', Auth::user()->school_id);  
            }
            if (Auth::user()->role_id == 10) {
            $all_staffs = $all_staffs->with('departments', 'designations', 'roles')->where('district_idFk',$smStaff->district_idFk)->where('role_id','!=', 10);  
            }
            else{
            $all_staffs = $all_staffs->with('departments', 'designations', 'roles');    
            }  
            $all_staffs = $all_staffs->get();
            ...
            $roles = $roles->where('is_saas', 0)
                ...
                ->orderBy('name', 'asc')
                ->get();  
            $sm_staf = DB::table('sm_staffs')->where(['role_id'=>Auth::user()->role_id,'user_id'=>Auth::user()->id])->pluck('district_idFk');
            if (Auth::user()->role_id == 10) {
                $school_info=DB::table('sm_schools')->whereIn('district_idFk',$sm_staf)->where('active_status',1)->get();    
            }elseif (Auth::user()->role_id == 11) {
                $school_info=DB::table('sm_schools')->where('id',Auth::user()->school_id)->where('active_status',1)->get(); 
            }
            else{
                $school_info = SmSchool::get();
            } 
            return view('backEnd.humanResource.staff_list', compact('all_staffs', 'roles','school_info'));
    ...
    public function addStaff()
    {
        ...
        try {
            ...
            $roles = InfixRole::where('is_saas', 0)->where('active_status', '=', 1);
                ...
                // })
            if ($role_id == 10) {
                $roles = $roles->whereNotIn('id', [10,5,1, 2, 3]);
            }if($role_id == 11){
                $roles = $roles->whereNotIn('id', [11,10,5,1, 2, 3]);
            }
            $roles = $roles->whereNotIn('id', [1, 2, 3])
                ->orderBy('name', 'asc')
                ->get();
            ...
            if ($role_id == 11) {
                $sm_stuff = $sm_stuff->where('sm_schools.id',$user->school_id);
                 
            }
            if ($role_id == 10) {
                $sm_stuff = $sm_stuff->where('district_idFk',$school[0]->district_idFk); 
            }
            $sm_stuff=$sm_stuff->get();
            $program = Program::get();
            return view('backEnd.humanResource.addStaff', compact('roles', 'departments', 'designations', 'marital_ststus', 'max_staff_no', 'genders', 'custom_fields','districts','sm_stuff','role_id','program'));
        } catch (\Exception $e) {
            return $e->getMessage();
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    public function staffStore(staffRequest $request)
    {
        ...
        // if ($request->school_name == '' && empty($request->school_name)) {
        //     return response()->json(['error'=>array('school_name'=>'required filed')]); exit;
        // }
        ...
        try {
            ...
            try {
                ...
                $staff->school_id = $request->school_name;
                $staff->staff_photo = fileUpload($request->staff_photo, $designation);
                $staff->gender_id = $request->gender_id;
                if ($request->role_id == 10) {
                    $staff->district_idFk = $school[0]->district_idFk;
                }else{
                    $staff->district_idFk = $request->district_name;
                }
                $staff->program_idFk = $request->program_name;
    ...
    public function searchStaff(Request $request)
    {
        ...
        try {
            ...
            if ($request->school_name != "") {
                $staff->where('school_id', $request->school_name);
            }
            if (Auth::user()->role_id != 1) {
                $staff->where('role_id', '!=', 1);
            }

            $all_staffs = $staff->get();
            $sm_staf = DB::table('sm_staffs')->where(['role_id'=>Auth::user()->role_id,'user_id'=>Auth::user()->id])->pluck('district_idFk');
            if (Auth::user()->role_id == 10) {
                $school_info=DB::table('sm_schools')->whereIn('district_idFk',$sm_staf)->where('active_status',1)->get(); 
            }else{
        ...
        try {
            ...
            if ($tables == null) {
                if (checkAdmin()) {
                    ...
                    $staffs = SmStaff::withOutGlobalScope(ActiveStatusSchoolScope::class)->where('id', $id)->first();
                }
