...
class SmStudentAdmissionController extends Controller
{
        ...
        if (($request->sibling_id == 0 || $request->sibling_id == 1) && $request->parent_id == "") {
            ...
            $request->validate(
                [
                    ...
                    'roll_number' => 'required',
                    'email_address' => 'required|unique:sm_students,email,'.$student_detail->id,
                    'class' => 'required',
                    'section' => 'required',
                    'gender' => 'required',
                    'first_name' => 'required|max:100',
    ...
    public function studentPromoteStore(Request $request)
    {
        ...
        try {
            ...
            $Upsessions = SmAcademicYear::where('active_status', 1)
                ->whereYear('created_at', '>', date('Y', strtotime($UpYear->year)) . ' 00:00:00')
                ->where('school_id', Auth::user()->school_id)
                ->get();
            ...
            if ($request->promote_class == "" || $request->promote_session == "") {
                $students = SmStudent::where('class_id', '=', $request->promote_class)
                    ->where('session_id', '=', $request->promote_session)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->get();
