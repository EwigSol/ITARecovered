...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (config('app.app_sync') && $request->auto_login){dd('ff');
            ...
            $user = User::where('email', $request->email)->first();     
             if (!$user || $user->email != $request->email) {
                 Toastr::error('Your Email do not match our records..', 'Failed');
        ...
        if ($logged_in) {
            ...
            if (Auth::user()->role_id == 2) {
                ...
                $profile = DB::table('sm_staffs')->where(['user_id'=>Auth::id(),'active_status'=>1,'school_id'=>$school->id])->first();   
                if ($profile) {
            ...
            // if(!$session_id){
            //     $session = SmAcademicYear::where('school_id', Auth::user()->school_id)->where('active_status', 1)->first()->id;
            // } else{
            ...
            $email_template = SmsTemplate::where('school_id',$school->id)->first();
